...
local function any_category_enabled()
	...
	for _, new_category in ipairs(new_categories_lookup) do
		any_new_category = any_new_category or mod:get(new_category.setting)
	end
...
mod:hook(ScoreboardUI, "update_topic_data", math_floor_hook)

mod:hook(ScoreboardUI, "set_player_list_data_by_index", function (func, self, topic_index)
	if not any_category_enabled() then
	...
	ScoreboardUI.set_player_widget_score_by_index = function (scoreboard_ui, player_index, score)
		...
		score = ""
		if player_index == 1 then
			score = statistics_db:get_stat(stats_id, heal_procs_category.data_stat_name)
		end
		if player_index == 2 then
			score = statistics_db:get_stat(stats_id, scavenger_procs_category.data_stat_name)
		end
...
local function on_proc(proc_type, amount)
	...
	local enabled = mod:get(heal_procs_category.setting) or mod:get(scavenger_procs_category.setting)
	if enabled then
	...
	local db_value = statistics_db:get_stat(stats_id, proc_category.data_stat_name)
	...
	statistics_db:set_stat(stats_id, proc_category.data_stat_name, melee.." / "..ranged.." / "..total)
end

--- regrowth tracking
mod:hook(DamageUtils, "buff_on_attack", function (func, unit, hit_unit, attack_type, ...)
	if not mod:get(PLAYER_PROCS) then
...
local function DeathReactions_start_hook(func, unit, dt, context, t, killing_blow, is_server, cached_wall_nail_data)
	...
	BuffExtension.apply_buffs_to_value = function (self, value, stat_buff)
		local amount, procced, parent_id = original_BuffExtension_apply_buffs_to_value(self, value, stat_buff)
		if procced and stat_buff == StatBuffIndex.HEAL_ON_KILL and mod:get(PLAYER_PROCS) then
			on_proc(PROC_TYPE_HP, amount)
		end
		return amount, procced, parent_id
	end
...
mod:hook(ScoreboardUI, "init", function (func, self, end_of_level_ui_context)
	if mod:get(SCOREBOARD_FIXED_ORDER) then
...
mod:hook(GenericUnitDamageExtension, "add_damage", function (func, self, attacker_unit, damage_amount, hit_zone_name, damage_type, damage_direction, damage_source_name, hit_ragdoll_actor, damaging_unit)
	...
	for _, new_category in ipairs(register_damage_categories) do
		local enabled = mod:get(new_category.setting)
		...
		if player and ScriptUnit.has_extension(self.unit, "health_system") and ScriptUnit.has_extension(self.unit, "buff_system") then
			...
			if player_attacker and self_ff_damage > 0 and damage_type ~= "wounded_dot" and damage_type ~= "knockdown_bleed" and damage_type ~= "kinetic" then
				...
				if actual_attacker_unit == self.unit then
					statistics_db:modify_stat_by_amount(stats_id, "ff_self", self_ff_damage)
				else
					statistics_db:modify_stat_by_amount(stats_id, "ff", self_ff_damage)
				end
			end
		end
	end

	if mod:get(DAMAGE_TAKEN) then
