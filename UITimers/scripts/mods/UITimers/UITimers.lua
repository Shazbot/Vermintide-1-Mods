-- luacheck: globals get_mod UIRenderer Managers UIResolution Colors UISceneGraph BuffTemplates
-- luacheck: globals UIPasses UnitFrameUI UIWidget UILayer
...
local function get_scenegraph_definition(owner)
	local indicator_position = mod:get(SETTING_TIMERS_POSITION)
...
local speedup_info = {
	...
	secondary_start_time = -math.huge,
...
local function notify_speedup_proc(unit, buff, params)
	if mod:is_enabled() and unit == Managers.player:local_player().player_unit then
...
local function notify_infinite_ammo_proc(unit, buff, params)
	if mod:is_enabled() and unit == Managers.player:local_player().player_unit then
		if speedup_info.secondary_end_time == -math.huge or (speedup_info.secondary_end_time + 10) < params.t then
			speedup_info.secondary_start_time = params.t
			speedup_info.secondary_duration = params.end_time - params.t
...
local function setup_speedup_indicator()
	...
	local damage_reduction_from_proc_buff = BuffTemplates.damage_reduction_from_proc.buffs[1]
	...
	local function set_buff_hook(buff_template, buff_hook_name, buff_hook_name_remove)
		assert((not buff_template.apply_buff_func) or buff_template.apply_buff_func == buff_hook_name)
		assert((not buff_template.remove_buff_func) or buff_template.remove_buff_func == buff_hook_name_remove)
		assert((not buff_template.reapply_buff_func) or buff_template.reapply_buff_func == buff_hook_name)

		buff_template.apply_buff_func = buff_hook_name
		buff_template.remove_buff_func = buff_hook_name_remove
		buff_template.reapply_buff_func = buff_hook_name
	end

	set_buff_hook(infinite_ammo_buff, "hudmod_notify_infinite_ammo_proc", "hudmod_notify_infinite_ammo_remove")
	set_buff_hook(damage_reduction_from_proc_buff, "hudmod_notify_infinite_ammo_proc", "hudmod_notify_infinite_ammo_remove")
...
mod:hook(UnitFrameUI, "draw", function(orig_func, self, dt)
	...
	if self._is_visible and self._hudmod_is_own_player then
		...
		local function update_widget(widget_name, info, widget_definition)
			...
			local end_time = info.secondary_end_time and math.max(info.end_time, info.secondary_end_time) or info.end_time
			if t >= end_time then
				return
			end
			...
			local duration = info.duration or (end_time - info.start_time)
			widget.content.progress_content.progress_value = (info.end_time - t) / duration

			-- debug visualization
			-- duration = 1
			-- widget.content.progress_content.progress_value = 0.5

			if info.secondary_end_time ~= nil and info.secondary_end_time ~= -math.huge then
				widget.content.secondary_progress_content.progress_value = (info.secondary_end_time - t) / info.secondary_duration
			else
				if widget.content.secondary_progress_content then
					widget.content.secondary_progress_content.progress_value = 0
				end
			end
